...
**Dragon Depth:**
- Evolution stage stat multipliers: Hatchling 1.0× → Titan 2.10×. Applied in `DragonInstance.Hp/Attack/Defense` via `EvolutionMultiplier` property. `GetEvolutionMultiplier(stage)` is public static for use elsewhere.
- `DragonInstance.UltimateSkill` returns `skillSet.ultimateSkill` only when `BondLevel >= 6`.
- `DragonTower` auto-casts the ultimate in waves (separate `_ultimateCooldown`/`_lastUltimateCastTime`), shows "ULTIMATE!" text indicator.
- 10 ultimate `SkillDefinition` assets generated by SceneBootstrapper at `Assets/ScriptableObjects/Skills/{id}_ultimate.asset`, wired into each `DragonDefinition.skillSet.ultimateSkill`.
- Global damage balance +15% via `PrototypeBalance.GlobalDamageBalance = 1.15f` (new field, applied in `DragonInstance.Attack`).

**Balance:**
- Starting resources: Mana 300→320, Gold 130→150.
- Upgrade base cost: 35→30.
- Chapter 1 Wave 01-05 gold/mana rewards bumped +10–17% for better early pacing.
- `PrototypeBalanceConfig` now always updated on Build Battle Scene (not just create-if-missing).

**Railway Deployment:**
- `Backend/Dockerfile` + `Backend/railway.toml` — deploys via Git push to Railway.
- `/health` GET endpoint returns `{ status, timestamp }` — Railway healthcheck target.
- Production startup auto-runs EF migrations (`db.Database.Migrate()` guarded by `IsProduction()`).
- `Backend/DragonTD.API/appsettings.Production.json` — empty placeholders; Railway injects via env vars: `ConnectionStrings__DefaultConnection = ${{Postgres.DATABASE_URL}}`, `Jwt__Secret = <32+ char secret>`.
- **User action required:** Create Railway project → add PostgreSQL service → set env vars → deploy. See plan Task 9 for step-by-step.

## Audio System + Passive Skills — 2026-05-31

**Audio:**
- `AudioManager` DontDestroyOnLoad singleton at `Unity/Assets/Scripts/Core/AudioManager.cs`.
- Two `AudioSource` components: SFX (`PlayOneShot`) + music (looping, 0.5s crossfade).
- 13 `SfxKey` values + 3 `MusicKey` values. Clips loaded from `Resources/Audio/` by filename — missing clip = silent, no crash.
- Volume + mute in `PlayerPrefs` (`audio_sfx_volume`, `audio_music_volume`, `audio_muted`).
- SFX wired: attack, enemy death/reach-base, wave start/end, upgrade, skill cast, ultimate, tower place/sell, button click, victory, defeat.
- Music: menu starts in MainMenu; battle plays on Wave state; victory sting on Victory; stop on Defeat.
- Drop audio files at `Unity/Assets/Resources/Audio/` with the filename keys. See `docs/art-generation-guide.md` for generation instructions.

**Passive On-Hit Skills:**
- `PassiveSkillType` enum: `None`, `SlowOnHit`, `BurnOnHit`, `PoisonOnHit`, `StunOnHit`, `AoeSplash`, `ChainLightning`.
- `SkillDefinition` has `passiveType`, `passiveChance`, `passiveChainCount`, `passiveSplashPercent` fields.
- `DragonTower.ApplyPassiveEffect(target)` called after every normal attack — checks `skillSet.passiveSkill`.
- `EnemyBase.ApplyStun(duration)` — temporarily zeroes `_moveSpeedMultiplier`, shows "STUN" text.
- 10 passive assets generated by SceneBootstrapper on Build Battle Scene:
  - Voltaris: ChainLightning (2 arcs, 50%) | Frostfang: SlowOnHit 20%/1.5s | Magmaclaw: BurnOnHit 5/s/2s
  - Tempest Glacion: AoeSplash 30%/r2 | Stonehide: StunOnHit 15%/0.8s | Celestara: AoeSplash 25%/r2.5
